...
<body>
	...
	<c:if test="${fn:contains(string1, 'No')}">
		<h4>No가 포함된 문자열입니다.</h4>
	</c:if>
	
	<h3>3. 특정 문자열로 시작, 끝 나는지 확인</h3>
	<h4>No로 시작하는가? ${fn:startsWith(string1, "No") }</h4>
	<h4>gain.으로 끝나는가? ${fn:endsWith(string1, "gain.") }</h4>
	
	<c:set var="string2" value="I love chicken"/>
	
	<h3>4. 문자열 치환</h3>
	<!-- string2의 문자열 자체가 바뀌는것이 아니라 리턴되는 문자열만 바뀌어져서 보여짐. -->
